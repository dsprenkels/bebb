...
instance Semigroup EmitState where
  es1 <> es2 = EmitState { offset = offset es2
                         , bin    = bin es1 <> bin es2
                         , cp     = cp es1 <> cp es2
...
emitInstr Instr { mnemonic = mnemonicN, opnds } span = do
  ...
  let mnemonicMatches =
        filter (\instr -> TD.mnemonic instr == mnemonic) TD.bebbInstrs
  ...
  if null mnemonicMatches
    then do
      ...
      modify (<> mempty { errs = [err] })
    else case filter (\instr -> Just (TD.operandDesc instr) == opndDesc) mnemonicMatches of
      [] -> do
        let msg = printf "invalid operands for '%s' instruction (%s)"
                         mnemonic
                         (show opndDesc)
